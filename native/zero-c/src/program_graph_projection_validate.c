...
static bool projection_graph_matches_store_nodes(const ZProgramGraphStore *store, const ZProgramGraph *graph, ZDiag *diag) {
...
bool z_program_graph_projection_store_matches_graph(const ZProgramGraphStore *store, ZDiag *diag) {
  ...
  if (ok) {
    ...
    ok = projection_graph_matches_store_nodes(store, &graph, diag);
  }
